...
public class WorkerExchangeAssignmentRule extends PRelOptRule {
  ...
  public PRelNode onMatch(PRelOptRuleCall call) {
    ...
    if (currentNodeExchange == null) {
      if (!derivedDistribution.satisfies(relCollation)) {
        ...
        PinotPhysicalExchange newExchange = new PinotPhysicalExchange(currentNode.getInput(0).getRelNode(),
            Collections.emptyList(), PinotPhysicalExchangeType.IDENTITY_EXCHANGE, relCollation);
        PinotDataDistribution newDataDistribution = derivedDistribution.withCollation(relCollation);
        currentNodeExchange = new PRelNode(_physicalPlannerContext.getNextId(), newExchange, newDataDistribution);
      }
    } else {
    ...
    if (isLeafStageBoundary) {
      ...
      return new PRelNode(_physicalPlannerContext.getNextId(), physicalExchange, currentNode.getPinotDataDistributionOrThrow(),
  ...
  public PRelNode onDone(PRelNode currentNode) {
    ...
    if (currentNode.getInputs().size() <= 1
        || currentNode.getPinotDataDistributionOrThrow().getType() != PinotDataDistribution.Type.HASH_PARTITIONED) {
      return currentNode;
    }
    ...
    for (int inputIndex = 1; inputIndex < currentNode.getInputs().size(); inputIndex++) {
      ...
      if (inputDistribution.getType() == PinotDataDistribution.Type.HASH_PARTITIONED) {
        ...
        if (inheritedDist.getType() == PinotDataDistribution.Type.HASH_PARTITIONED) {
          newDistributionSet.addAll(inheritedDist.getHashDistributionDesc());
        }
      }
    }
    PinotDataDistribution finalDist = new PinotDataDistribution(PinotDataDistribution.Type.HASH_PARTITIONED,
  ...
  private PRelNode meetDistributionConstraint(PRelNode currentNode, PinotDataDistribution currentNodeDistribution,
    ...
    if (distributionConstraint.getType() == RelDistribution.Type.BROADCAST_DISTRIBUTED) {
      ...
      PinotDataDistribution pinotDataDistribution = new PinotDataDistribution(PinotDataDistribution.Type.BROADCAST,
          currentNodeDistribution.getWorkers(), currentNodeDistribution.getWorkerHash(), null, null);
      return new PRelNode(_physicalPlannerContext.getNextId(), physicalExchange, pinotDataDistribution);
    }
    ...
    if (distributionConstraint.getType() == RelDistribution.Type.HASH_DISTRIBUTED) {
      PinotPhysicalExchange physicalExchange = new PinotPhysicalExchange(currentNode.getRelNode(),
          distributionConstraint.getKeys(), PinotPhysicalExchangeType.PARTITIONING_EXCHANGE);
      ...
      PinotDataDistribution pinotDataDistribution = new PinotDataDistribution(
          PinotDataDistribution.Type.HASH_PARTITIONED, currentNodeDistribution.getWorkers(),
  ...
  private PRelNode meetParentEnforcedDistributionConstraintNew(PRelNode currentNode, RelDistribution relDistribution,
    ...
    if (relDistribution.getType() == RelDistribution.Type.RANDOM_DISTRIBUTED
      ...
      PinotPhysicalExchange physicalExchange = new PinotPhysicalExchange(currentNode.getRelNode(),
          distributionKeys, PinotPhysicalExchangeType.PARTITIONING_EXCHANGE);
      ...
      return new PRelNode(_physicalPlannerContext.getNextId(), physicalExchange, newDistribution);
      ...
      PinotPhysicalExchange physicalExchange = new PinotPhysicalExchange(currentNode.getRelNode(),
          Collections.emptyList(), PinotPhysicalExchangeType.BROADCAST_EXCHANGE);
      ...
      PinotPhysicalExchange physicalExchange = new PinotPhysicalExchange(currentNode.getRelNode(),
          Collections.emptyList(), PinotPhysicalExchangeType.SINGLETON_EXCHANGE);
    ...
    PinotPhysicalExchange pinotPhysicalExchange = new PinotPhysicalExchange(currentNode.getRelNode(),
        relDistribution.getKeys(), PinotPhysicalExchangeType.PARTITIONING_EXCHANGE, null);
    ...
    return new PRelNode(_physicalPlannerContext.getNextId(), pinotPhysicalExchange, pinotDataDistribution);
  }
}
