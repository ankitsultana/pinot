...
public class BrokerRoutingManager implements RoutingManager, ClusterChangeHandler {
  ...
  public RoutingTable getRoutingTable(BrokerRequest brokerRequest, long requestId) {
    ...
    Map<String, ServerInstance> segmentToServerMap = new HashMap<>();
    for (Map.Entry<String, String> entry : segmentToInstanceMap.entrySet()) {
      ...
      if (serverInstance != null) {
        ...
        segmentToServerMap.put(entry.getKey(), serverInstance);
      } else {
    ...
    return new RoutingTable(serverInstanceToSegmentsMap, segmentToServerMap, selectionResult.getUnavailableSegments(),
        selectionResult.getNumPrunedSegments(),
        routingEntry.calculatePartitionInfo(selectionResult.getSegmentToInstanceMap().keySet()));
  }
