...
public class LocalJoinOperator extends BaseOperator<SelectionResultsBlock> {
  ...
  private final BaseProjectOperator<ProjectionBlock> _leftTable;
  ...
  public LocalJoinOperator(IndexSegment indexSegment, QueryContext queryContext, BaseProjectOperator leftTable,
  ...
  protected SelectionResultsBlock getNextBlock() {
    ...
    LookUpContext context = _queryContext.getOrComputeSharedValue(LookUpContext.class, "", k -> {
      ...
      for (int i = 0; i < _leftProjectColumns.length; i++) {
        ...
        ColumnContext columnMetadata = _leftTable.getResultColumnContext(leftProjectColumn);
        resultColumnTypes[i] =
    ...
    ProjectionBlock transformBlock;
